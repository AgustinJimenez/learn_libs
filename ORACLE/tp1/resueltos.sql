...
SELECT COUNT(*) CANTIDAD_PRESAMOS_ACTIVOS FROM PRESTAMOS WHERE FECHA_CANCEL IS NOT NULL 


/*

SELECT ( LINE ||'/'|| POSITION ||' '|| TEXT ) ERROR FROM SYS.USER_ERRORS WHERE NAME = 'TRIGGER_INSERT_PRESTAMOS' AND TYPE = 'TRIGGER';

*/
SELECT * FROM PRESTAMOS

DELETE FROM PRESTAMOS WHERE NRO_PRESTAMO = 2

SELECT * FROM CUOTAS

INSERT INTO PRESTAMOS VALUES( 2, 1, 1, SYSDATE, 5000000, 15, NULL, NULL )


CREATE OR REPLACE TRIGGER
	TRIGGER_INSERT_PRESTAMOS 
BEFORE INSERT ON
	PRESTAMOS
FOR EACH ROW

DECLARE
	V_CANT_PREST_ACTIVOS NUMBER := 0;
	V_INGRESOS_TOTALES NUMBER := 0;
	V_PATRIMONIO NUMBER := 0;
	V_ANTIGUEDAD NUMBER := 0;
	V_ANTIGUEDAD_MINIMA NUMBER := 0;
BEGIN
	DBMS_OUTPUT.PUT_LINE( :NEW.NRO_PRESTAMO||' '|| :NEW.ID_SOCIO||' '|| :NEW.CODIGO_TIPO||' '|| :NEW.FECHA_PRESTAMO||' '|| :NEW.MONTO_PRESTAMO||' '|| :NEW.PLAZO_EN_MESES||' '|| :NEW.FECHA_CANCEL||' '|| :NEW.TIPO_CANCEL  );
	--cantidad de prestamos donde el socio no tiene fecha de cancelacion
	SELECT COUNT(*) INTO V_CANT_PREST_ACTIVOS FROM PRESTAMOS WHERE FECHA_CANCEL IS NOT NULL AND ID_SOCIO = :NEW.ID_SOCIO;
	-- ingresos totales del socio
	V_INGRESOS_TOTALES := PCK_PRESTAMO.F_CALCULAR_INGRESO( :NEW.ID_SOCIO );
	-- patrimonio total del socio
	V_PATRIMONIO := PCK_PRESTAMO.F_CALCULAR_PATRIMONIO( :NEW.ID_SOCIO );
	-- si el socio tiene otros perestamos pendientes raise error
	IF V_CANT_PREST_ACTIVOS > 0 THEN
		RAISE_APPLICATION_ERROR (-20001, 'EL CLIENTE TIENE PRESTAMOS ACTIVOS');
	ELSE
	
		IF V_INGRESOS_TOTALES < ( 0.5 *  ) THEN
		
		ELSE
			
			IF V_PATRIMONIO > ( :NEW.MONTO_PRESTAMO * 0.3) THEN
				SELECT * FROM PARAMETROS;
				RAISE_APPLICATION_ERROR (-20001, 'PATRIMONIO DE CLIENTE NO ES MAYOR AL 30% DE LA CUOTA');
			ELSE
				SELECT FLOOR( MONTHS_BETWEEN( SYSDATE , FECHA_INGRESO )/12 ) INTO V_ANTIGUEDAD FROM SOCIO WHERE ID_SOCIO = :NEW.ID_SOCIO;
				SELECT ANTIGUEDAD_MINIMA INTO V_ANTIGUEDAD_MINIMA FROM PARAMETROS;
				
				IF V_ANTIGUEDAD < V_ANTIGUEDAD_MINIMA THEN
					RAISE_APPLICATION_ERROR (-20001, 'NO SE CUMPLE CON LA ANTIGUEDAD MINIMA');
				ELSE
	...
	RAISE_APPLICATION_ERROR (-20001, 'ENDING TRIGGER EXCEC');
